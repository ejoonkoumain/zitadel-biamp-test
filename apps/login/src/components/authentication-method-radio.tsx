"use client";

import { Box, FormControlLabel, Radio, RadioGroup, Stack, Typography } from "@mui/material";
import { Translated } from "./translated";

export enum AuthenticationMethod {
  Passkey = "passkey",
  Password = "password",
}

export const methods = [AuthenticationMethod.Passkey, AuthenticationMethod.Password];

// A native MUI RadioGroup/Radio pair rather than ToggleButtonGroup: the
// previous headlessui RadioGroup gave native ARIA radiogroup semantics with
// roving-tabindex arrow-key navigation between options. MUI's
// ToggleButtonGroup does not provide that (each button is its own tab stop);
// RadioGroup + native <input type="radio"> keeps the same keyboard behaviour
// for free via the browser, while the tile look is achieved by styling
// FormControlLabel as the clickable card and visually hiding the radio dot.
export function AuthenticationMethodRadio({
  selected,
  selectionChanged,
}: {
  selected: any;
  selectionChanged: (value: any) => void;
}) {
  return (
    <Box width="100%">
      <Box sx={{ mx: "auto", width: "100%", maxWidth: 448 }}>
        <RadioGroup
          row
          value={selected}
          onChange={(e) => selectionChanged(e.target.value)}
          sx={{ flexWrap: "nowrap", width: "100%", gap: 2 }}
        >
          {methods.map((method) => (
            <FormControlLabel
              key={method}
              value={method}
              control={<Radio data-testid={method + "-radio"} sx={{ position: "absolute", opacity: 0 }} />}
              label={
                <Stack alignItems="center" sx={{ textAlign: "center" }}>
                  {method === "passkey" && (
                    <Box
                      component="svg"
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
                      viewBox="0 0 24 24"
                      strokeWidth="1.5"
                      stroke="currentColor"
                      sx={{ width: 32, height: 32, mb: 1.5 }}
                    >
                      <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        d="M7.864 4.243A7.5 7.5 0 0119.5 10.5c0 2.92-.556 5.709-1.568 8.268M5.742 6.364A7.465 7.465 0 004.5 10.5a7.464 7.464 0 01-1.15 3.993m1.989 3.559A11.209 11.209 0 008.25 10.5a3.75 3.75 0 117.5 0c0 .527-.021 1.049-.064 1.565M12 10.5a14.94 14.94 0 01-3.6 9.75m6.633-4.596a18.666 18.666 0 01-2.485 5.33"
                      />
                    </Box>
                  )}
                  {method === "password" && (
                    <Box
                      component="svg"
                      fill="currentColor"
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 24 24"
                      sx={{ width: 32, height: 32, mb: 1.5 }}
                    >
                      <title>form-textbox-password</title>
                      <path d="M17,7H22V17H17V19A1,1 0 0,0 18,20H20V22H17.5C16.95,22 16,21.55 16,21C16,21.55 15.05,22 14.5,22H12V20H14A1,1 0 0,0 15,19V5A1,1 0 0,0 14,4H12V2H14.5C15.05,2 16,2.45 16,3C16,2.45 16.95,2 17.5,2H20V4H18A1,1 0 0,0 17,5V7M2,7H13V9H4V15H13V17H2V7M20,15V9H17V15H20M8.5,12A1.5,1.5 0 0,0 7,10.5A1.5,1.5 0 0,0 5.5,12A1.5,1.5 0 0,0 7,13.5A1.5,1.5 0 0,0 8.5,12M13,10.89C12.39,10.33 11.44,10.38 10.88,11C10.32,11.6 10.37,12.55 11,13.11C11.55,13.63 12.43,13.63 13,13.11V10.89Z" />
                    </Box>
                  )}

                  <Typography variant="body2">
                    {method === AuthenticationMethod.Passkey && (
                      <Translated i18nKey="methods.passkey" namespace="register" />
                    )}
                    {method === AuthenticationMethod.Password && (
                      <Translated i18nKey="methods.password" namespace="register" />
                    )}
                  </Typography>
                </Stack>
              }
              sx={{
                position: "relative",
                m: 0,
                flex: 1,
                borderRadius: 2,
                border: 1,
                borderColor: "divider",
                px: 2.5,
                py: 2,
                "&:hover": { boxShadow: 4 },
                "&:has(.Mui-focusVisible)": {
                  outline: (theme) => `2px solid ${theme.palette.primary.main}`,
                  outlineOffset: 2,
                },
                "&:has(.Mui-checked)": {
                  borderColor: "primary.main",
                  boxShadow: (theme) => `0 0 0 1px ${theme.palette.primary.main}`,
                },
              }}
            />
          ))}
        </RadioGroup>
      </Box>
    </Box>
  );
}
